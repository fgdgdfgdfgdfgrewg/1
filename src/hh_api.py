...
def search_employers_by_name(names: Iterable[str]) -> list[dict]:
    """По имени возвращает словари работодателей (берёт первый релевантный результат на имя)."""
    found: list[dict] = []
    for name in names:
        if not name:
            continue
        resp = requests.get(f"{settings.hh_base}/employers", params={"text": name, "only_with_vacancies": False}, headers=HEADERS, timeout=30)
...
def iter_vacancies_by_employer(employer_id: int, per_company_limit: int = 100) -> Iterator[dict]:
